...
import { Article, Assignment, ContactPage, FileCopy, Home, Person, Shield } from "@mui/icons-material"
        ...
        <>
            ...
            {keycloak.authenticated && menuItems.length > 0 && menuItems.map(item => (
                ...
                < ListItemButton key={item.key} component={Link} to={`/${item.key}`} onClick={closeMenu}>
                    <ListItemIcon>
                        {
                            item.key === "clinicalhistoryformat" ?
                                <FileCopy />
                                :
                                item.key === "clinicalhistory" ?
                                    <Article />
                                    :
                                    item.key === "patientformat" ? <ContactPage />
                                        :
                                        item.key === "patient" ? <Person />
                                            :
                                            item.key === "role" ? <Shield />
                                                :
                                                <Assignment />
                        }
                    </ListItemIcon>
